...
//
//     const [user, setUser] = useState(null);
//     const [accessToken, setAccessToken] = useState(null); // store only access token
//     const [returnPath, setReturnPath] = useState("/");
//
//     const isAuthenticated = !!user;
//
...
export const AuthProvider = ({ children }) => {
    ...
    const refreshAccessToken = async () => {
        try {
            ...
            setUser(response);
            return response.token || null; // if your backend sends new token
        } catch (err) {
            setUser(null);
            return null;
        }
    ...
    const login = async (email, password) => {
        try {
            Swal.fire({ title: "Checking credentials...", didOpen: () => Swal.showLoading(), allowOutsideClick: false });

            const loggedUser = await authService.login(email, password);

            Swal.close();
            setUser(loggedUser);
            setAccessToken(loggedUser.token);

            await MySwal.fire({ toast: true, position: "top-end", showConfirmButton: false, timer: 2000, icon: "success", title: `Welcome back, ${loggedUser.name}!`, background: "#fff", iconColor: "green" });

            if (loggedUser.role === "cashier") navigate("/cashier");
            else navigate(returnPath);

            return true;
        } catch (err) {
            Swal.close();
            await MySwal.fire({ toast: true, position: "top-end", showConfirmButton: false, timer: 2500, icon: "error", title: err?.message || "Login failed", background: "#fff", iconColor: "red" });
            return false;
        }
    };
    ...
    const logout = async () => {
        setUser(null);
        setAccessToken(null);
        await MySwal.fire({ toast: true, position: "top-end", showConfirmButton: false, timer: 2000, icon: "info", title: "Logged out successfully", background: "#fff", iconColor: "blue" });
        navigate("/login");
    };

    return (
        <AuthContext.Provider value={{ user, accessToken, isAuthenticated, login, logout, refreshAccessToken }}>
            {children}
        </AuthContext.Provider>
    );
};
